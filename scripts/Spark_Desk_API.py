...
if __name__ == '__main__':
    ...
    anno_all = {}
    anno_01 = []
    final_file = open('./log/Spark_Desk_API_v2.1.txt', 'a', encoding='utf-8')
    ...
    for i in range(1, 31):
        ...
        function = ''
        for n in range(len(qad['tool_set'])):
            function = function + str(n + 1) + "、" + qad['tool_set'][n] + "\n"
        function = qad['prompt_prefix'] + "\n" + function
        prompt_suffix = qad['prompt_suffix']
        action_prompt = prompt.sys_prompt_function().format(question=instruction, function=function, prompt_suffix=prompt_suffix)
