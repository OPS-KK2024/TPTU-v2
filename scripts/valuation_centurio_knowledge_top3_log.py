...
if __name__ == '__main__':
    ...
    log_name = "{}_{}_{}_knoeledge.json".format(config["llm_name"], config["log_name"], datetime.now().strftime('%Y_%m_%d_%H_%M_%S'))
    ...
    for j in range(1):
        ...
        for i in range(question_range[0], question_range[1]):
            ...
            anno = {}
            anno["Q" + str(i)] = {}
            answers = qad['gt_plan_answer']
            anno["Q" + str(i)]["question"] = instruction
            anno["Q" + str(i)]["gt_plan_API"] = answers
            # final_action = agent(instruction)
            if "都不相似" not in answer.split('||')[1]:
                llm_answer = answer.split('||')[0].split('["')[1].split('"]')[0].split('", "')
            else:
                llm_answer = ["都不相似"]
            # if len(answers) <= len(llm_answer):
            #     for llm_data in llm_answer:
            #         if llm_data in answers:
            ...
            if answers == llm_answer or (i == 15 and "视图源管理-抓拍分析" in llm_answer):
                true_value = True
                true_value_num = true_value_num + 1
            else:
                if "都不相似" in answer.split('||')[1] and '|'.join(answers).count('不相似') == 3:
                    true_value = True
                    true_value_num = true_value_num + 1
                else:
                    true_value = False
            anno["Q" + str(i)]["return_plan_text"] = str(answer.split('||')[1])
