from autotask.prompt.prompt_centurio_knowledge_top3 import Centurio_Prompt
...
from autotask.utils.knowledge import Example
...
class CenturioAgent():
    ...
    def __call__(self, instruction: str, statement: str, example: str, answer: str) -> str:
        if 'mode' in self.config:
            if self.config['mode'] == 'test':
                ...
                while True:
                    ...
                    try:
                        print("================================START================================")
                        action_prompt = self.prompt.sys_prompt().format(question=instruction, statement=statement)
                        print(action_prompt)
                        ...
                        final_answer_01 = str_action
                        if "都不相似" in str_action:
                            final_answer = str(['都不相似'])
                        else:
                            for i in range(len(examples)):
                                if examples[i] in str_action or "语句" + str(i+1) in str_action:
                                    final_answer = answers[i]
                                    print(final_answer)

                        if not self.q_response is None:
                            self.q_response.put(final_answer)
                        else:
                            format_err += 1
                            print(format_err)
                        ...
                        #         print('---------------------------------------')
                        #         # 调用第二次的LLN输出模块名称
                        #         format_prompt = self.prompt.format_prompt().format(info=str_action)
                        #         # 调用llm，返回执行结果
                    ...
                    # 捕获其他错误
                        ...
                        print(error)
                # 函数返回，最终plan的结果
                return final_answer + '||' + final_answer_01
            else:
