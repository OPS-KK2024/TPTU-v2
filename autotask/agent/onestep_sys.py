...
class OneStepAgent():
    def __init__(self, config: dict, q_response: queue) -> None:
        self.config = config
            ...
            if self.config['mode'] == 'test':
                ...
                while True:
                    ...
                    try:
                        ...
                        # 中间结果保存在队列里面
                        self.q_response.put('ACTION: ' + str(actions))
                        # 将规划的结果转化为list格式
                        # 捕获格式出错/语法错误的问题
                        ...
                        self.q_response.put("循环次数为：" + str(retry) + ";其中语法错误为: " + str(format_err) + ";工具错误为: " + str(
                            tool_err) + ";其他错误为: " + str(other_err))
                        self.q_response.put("最终答案为：" + str(final_answer))
                        break

                    # 捕获语法错误
                    except (SyntaxError, AttributeError):
                        print("**********语法错误**********")
                        self.q_response.put("**********语法错误**********")
                        format_err += 1
                        print(format_err)
